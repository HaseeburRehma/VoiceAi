<template>
  <UCard class="max-w-sm w-full">
    ...
    <UForm ref="form" :schema="signUpSchema" :state="state" class="space-y-4 my-6" @submit="onSignUp">
      <UFormField name="name" label="Name" required>
        <UInput v-model.trim="state.name" placeholder="Enter your name" icon="i-lucide-user" class="w-full" />
      </UFormField>

      <UFormField name="username" label="Username" required>
        <UInput v-model.trim="state.username" placeholder="Enter you username" icon="i-lucide-at-sign" class="w-full" />
      </UFormField>
      <UFormField name="email" label="Email" required>
        <UInput v-model="state.email" placeholder="you@example.com" type="email" />
      </UFormField>
      <UFormField name="password" label="Password" required>
        <UInput v-model.trim="state.password" class="w-full" icon="i-lucide-key-round" placeholder="Enter your password"
          :type="showPassword ? 'text' : 'password'" :ui="{ trailing: 'pe-1' }">
          <template #trailing>
            <UButton aria-controls="password" color="neutral" variant="link"
      ...
      <UButton block class="mt-2" trailing-icon="i-lucide-arrow-right" type="submit" :disabled="loading"
    ...
    <div class="text-sm font-medium text-gray-500 dark:text-gray-300 text-center">
      ...
      <ULink class="text-[var(--ui-primary)] hover:text-[var(--ui-primary)]/75" to="/sign-in">
        Sign in here
      </ULink>
    </div>
  </UCard>
  ...
  try {
    await $fetch("/api/auth/signup", {
      ...
      body: {
        ...
        email:    event.data.email,    // ← add this!
